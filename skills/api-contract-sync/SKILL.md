...
- Spring com `springdoc-openapi` configurado e endpoint `/v3/api-docs` acessível (ou OpenAPI exportado em arquivo).
  - **Configuração do backend** (springdoc, annotations, contrato de erro): ver `spring-boot-engineer/references/openapi.md`. Esta skill assume o spec já produzido e cuida só da sincronização no front.
