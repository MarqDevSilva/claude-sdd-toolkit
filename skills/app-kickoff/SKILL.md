...
**Objetivo**: preencher `.spec/discovery/brief.md` (estrutura em [references/brief.template.md](references/brief.template.md)) com contexto fundamental antes de detalhar módulos.
...
**Saída**: `.spec/discovery/brief.md` consolidado + termos novos sugeridos pro glossário do projeto. Confirmar antes da Fase 2.
...
4. **Crie o arquivo** `.spec/discovery/module_<nome>.md` usando a estrutura padrão (resumo abaixo; versão completa em [references/modules.template.md](references/modules.template.md)).
...
1. **Decisões Transversais primeiro.** Antes de modelar entidades, conduzir as decisões da seção 1 do [references/entities.template.md](references/entities.template.md) via `AskUserQuestion` (em batch ou em rodadas curtas):
...
**Objetivo**: preencher `.spec/discovery/roadmap.md` (estrutura em [references/roadmap.template.md](references/roadmap.template.md)).
...
- Sugestão: atualizar o glossário do projeto com termos novos.
...
O glossário do projeto (quando existir) reúne os termos do domínio — sugira adicionar termos novos descobertos em cada fase.
...
- ❌ Concatenar vários módulos num único arquivo (use **um `module_<nome>.md` por módulo**).
